...
handled by `stripGrob()`). Here's a plot to use for exploration:
...
```{r display-titlegrob, out.width = "50%"}
gp <- gpar(fontsize = 20)
...
object which has a text grob as a child.
...
```{r display-titlegrob-wider-margins, out.width = "50%"}
display_tg(
  "pineapple",
  hjust = 0,
  vjust = 0,
...
```{r titlegrob-expand, out.width = "50%"}
display_tg(
  "pineapple",
  hjust = 0,
  vjust = 0,
...
that the behavior is only explicitly defined when `angle` is 0, 90, 180, or 270.
